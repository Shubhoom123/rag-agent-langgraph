...
def fetch_wikipedia(topic: str, retries: int = 3) -> Document | None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            return Document(
                page_content=page.content,  # Full article — no char cap
                metadata={
...
# ---------------------------------------------------------------------------
def seed_curated(vectorstore, config: dict, seeded: set) -> int:
...
# ---------------------------------------------------------------------------
def seed_huggingface(vectorstore, config: dict, seeded: set) -> int:
    ...
    for article in dataset:
        ...
        if processed % 100 == 0:
            print(f"  ✓ {processed:,}/{remaining:,} articles | {total_chunks:,} chunks")
...
def seed():
    ...
    chunks_1 = seed_curated(vectorstore=get_vectorstore(), config=config, seeded=seeded)

    # Reload seeded after phase 1
    seeded = load_seeded()
    chunks_2 = seed_huggingface(vectorstore=get_vectorstore(), config=config, seeded=seeded)
